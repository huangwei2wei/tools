...
function loading(id,big)
{
	if(big=='big')
	{
		var loa='<table class="loading" width="100%" height="100%" border="0" cellpadding="0" cellspacing="0"><tr><td align="center" valign="middle"><img  style="top:45%" src="./template/default/aa_images/frame/min/loading45.gif" /></td></tr></table>';
	}else{
		var loa='<table class="loading" width="100%" height="100%" border="0" cellpadding="0" cellspacing="0"><tr><td align="center" valign="middle"><img  style="top:45%" src="./template/default/aa_images/frame/min/loading45.gif" /></td></tr></table>';
	}
...
function ajaxsends(url,fun,method,fromid,id,vars)
{
	...
	{
		try{
			var fromobj=document.getElementById(this_.fromid); 
		}catch(err){
			var  fromobj=document.getElementsByName(this_.fromid)[0]; 
		}
		if(!fromobj){alert("表单不存在");return false;}
	}
...
{
	body().scrollTop=t;
	var upn=getCookie("_sys_novels_tmpdata");
	upn=json_x(upn);
	var sdata=upn.sdata?upn.sdata:""
	var my_data=document.getElementById('my_data').innerHTML;
	my_data=my_data.replace_all(sdata,"<font  style='background-color:#39F;'>"+sdata+"</font>");
	document.getElementById('my_data').innerHTML=my_data;
}
...
function movediv(url,title,css,cajax,scrolling,top,left,cssid,callback,parameter)
{
	...
	if(document.getElementById(cssid))
	{
		...
		var oTest = document.body;
		var newNode = document.createElement("div");
		newNode.setAttribute("id","movediv");
		oTest.appendChild(newNode);
	}

	if(!scrolling){scrolling='auto';}
	
	var infos='<div id="move_div_info" style="text-align:center;height:100%;"><\/div>';
	...
	'<div class="frame-a">'+
        ...
        '<div class="f-am1"><div style="position:absolute;top:5px;right:5px;"><img style="cursor:pointer;" onclick="logoutdiv(\''+cssid+'\')" src="'+imgp+'close.gif" /></div><\/div>'+
	...
	'<\/div>'+
	'<div class="frame-a">'+
    	'<div class="w24" style="height:4px; line-height:4px;font-size:2px;"><img src="'+imgp+'bot-1.gif" /><\/div>'+
        '<div class="f-am2" style="height:4px; line-height:4px;font-size:2px;"></div>'+
        '<div class="w24" style="height:4px; line-height:4px;font-size:2px;"><img src="'+imgp+'bot-3.gif" /><\/div>'+
    '<\/div>'
'<\/div>';
	...
	if(!parseInt(cajax))
	{
		...
		var tmpid="tmpid_"+timestamp;
		
		document.getElementById("move_div_info").innerHTML="<iframe id='"+tmpid+"' hspace='0' vspace='0' frameborder='0' scrolling='"+scrolling+"' height='100%' width='100%'><\/iframe>";
		$(tmpid).src=url;
	}
}
...
function small_movediv(url,css,top,left,cssid)
{
	...
	document.getElementById(cssid).innerHTML = '<div id=movediv_sm class="min-wod">'+
		'<div class="min-wod-a" style="position:relative;Z-INDEX: 998;"><img src="/template/default/f/image/open/bg-min-top.gif" /><div style="position:absolute;Z-INDEX: 999; top:5px;right:5px;"><img style="cursor:pointer;" onclick="logoutdiv(\''+cssid+'\')" src="/template/default/f/image/open/frame/close.gif" /></div></div>'+
...
function logoutdiv(cssid)
{
	...
	if(!fr_doc.getElementById("building").value || 
		(fr_doc.getElementById("building").value && !document.getElementById((fr_doc.getElementById("building").value)))
	)
	{
		var gezi=fr_doc.getElementById("gezi").value;
		var gezis=gezi.split(",");
		for(var i=0;i<gezis.length;i++)
		{
			if(document.getElementById(gezis[i]))
			{
				var aa=document.getElementById(gezis[i]).style.backgroundImage.split("c.png");
				if(aa.length==2)
				{
					document.getElementById(gezis[i]).style.backgroundImage = "url(a.png)";
				}
			}
		}
	}
