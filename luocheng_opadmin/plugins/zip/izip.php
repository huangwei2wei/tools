...
class izip extends module
{
	...
	function b_zip()
	{
		...
		<a href='/res/".$this->getmysqldb()."/pydata.zip' >pydata下载zip</a></div>";
	}
	...
	//初始化商城
	function ismail($mongo)
	{
		//$data=dbarr("SELECT * FROM `base3_mall` WHERE `mall_type`=1 and `type` in (6,8) ORDER BY `id`  ASC");
		$data=dbarr("SELECT * FROM `base3_mall` ORDER BY `id`  ASC");
		$mongo->mongo->delete("autoid_system",array("collection"=>"mall"));
		$mongo->mongo->delete("mall");
		foreach($data as $value)
		{
			$v=array(
				"id"=>intval($value["id"]),
				"mall_type"=>intval($value["mall_type"]),
				"sort"=>intval($value["sort"]),
				"goods_ext_id"=>intval($value["goods_ext_id"]),
				"type"=>intval($value["type"]),
				"money"=>intval($value["money"]),
				"money_type"=>intval($value["money_type"]),
				"sum"=>intval($value["sum"]),
				"stint_sum"=>intval($value["stint_sum"]),
				"colour"=>$value["colour"],
				"lv"=>intval($value["lv"]),
				"rand_p"=>floatval($value["rand_p"]),
				"rebate"=>floatval($value["rebate"]),
				"status"=>intval($value["status"]),
  			);
			$mongo->mongo->insert("mall",$v);
		}
	...
	function initialization($mongo,$post,$session)
	{
		...
		$mongo->mongo->update("code",array("status"=>0,"user_id"=>0),array("status"=>array('$ne'=>0)));
