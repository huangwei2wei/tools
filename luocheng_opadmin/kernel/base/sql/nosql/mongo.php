...
class mongo
{
    ...
    */  
	...
	function delete($table,$query=array(), $option=array())
	{   
         
		$this->selecttable($table);
		return $this->table->remove($query,$option); 
	}
	...
	* @access    Public
	* @param    string    table      表名
	* @param    list    groupkey      		分组key["a","b"]
	* @param    list    group      		分组之后的选取规则 $group=array("time"=>array('$min'=>'$time'),"goods_ext_id"=>array('$first'=>'$goods_ext_id'));
	http://docs.mongoing.com/manual-zh/reference/operator/aggregation/group.html#pipe._S_group		(group资料)
	http://docs.mongoing.com/manual-zh/reference/operator/aggregation.html#aggregation-pipeline-operator-reference（aggregate资料）
	* @param    list    countkeys     要统计的字段    ["a","b"]
	* @param    dict    query        查询条件    {'author':"dave"}
	* @param    dict    show        显示类容    {'author':1}
	* @param    dict    sort        排序   {'title':1,'author':1}
	...
	* @param    int    mtype    统计类型 1统计记录数 不为1统计字段值总数
	*/
	function new_aggregate($table,$groupkey,$group=false,$countkeys=false,$query=false,$show=false,$sort=false,$skip=0,$limit=0,$mtype=1)
	{
		...
		$_group=array('_id'=>array());
		foreach($groupkey as $key =>$value){
			$_group["_id"][$value]='$'.$value;
		}
		
		if($countkeys){
			if($mtype==1){
				foreach($countkeys as $key =>$value){
					$_group[$value]= array('$sum'=>$mtype);
				}
			}else{
				foreach($countkeys as $key =>$value){
					$_group[$value]= array('$sum'=>'$'.$value);
				}
			}
		}
		...
		$aggregatedata[]=array('$group'=>$_group);
	 ...
	 * @return	array	res		数据结果
	 */
	function ret_count($table,$query=array())
	{
		$this->selecttable($table);
	 ...
	 * @return	array	res		数据结果
	 */
	function findsort($table,$query=array(),$sort=array())
	{
		$this->selecttable($table);
		//$this->table->findOne(); //返回table集合中全部文档
		return $this->table->find($query)->sort($sort);
	}
