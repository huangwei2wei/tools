...
class mysql implements allsql
{
	...
	function conn($dbhost,$dbuser,$dbpw,$dbname)
	{
	    ...
	    $conn=@mysql_connect ($dbhost, $dbuser, $dbpw); //全部使用短链接
	...
	function sql($sql)
	{
		...
		$mysql_query=mysql_query($sql,$this->conn);
	...
	function get_insert_id()
	{
		$row=sql("SELECT ROW_COUNT( )");
		$rs=$this->dbarr($row);
		$sum=mysql_insert_id($this->conn)+$rs[0];
		return $sum<=0?0:$sum;
	}
	...
	function lastlyid($table,$field)
	{
		$row=sql("select ".$field." from ".$table." where `".$field."` is null");
		$rs=$this->dbarr($row);
		return $rs[$field];
	}
	...
	function dbfield_note($table,$dbsel=false)
	{
	  ...
	  if($dbsel)
	  {
	  	$ku=$config_storage["MYSQL"][$dbsel]["db"];
	  }else{
	  	$ku=$config_storage["MYSQL"]["base"]["db"];
	  }
	  ...
	  CHARACTER_SET_NAME,IS_NULLABLE,COLUMN_DEFAULT,ORDINAL_POSITION
	  FROM INFORMATION_SCHEMA.COLUMNS
	  WHERE table_name = '".$table."' and TABLE_SCHEMA='$ku'";
