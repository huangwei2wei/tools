...
class mongopage{
	...
	function mongodb($mongodb,$table,$query,$sort,$show=array())
	{	
		
		$total_row=$mongodb->find($table,$query)->count();
		$this->total_row=$total_row;
				
		$pages=intval($total_row/$this->pagesize); //算出多少页
		
		if ($total_row%$this->pagesize)
		...
		return restoarray($mongodb->find($table,$query,$show)->sort($sort)->skip(intval($offset))->limit(intval($this->pagesize)));
	}
	...
	function p($return=false,$allurl='')
	{
			...
			if(!$allurl)
			{
				...
				$allurl=str_replace('&page='.$page,'',$allurl);
