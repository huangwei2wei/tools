...
function open_flash_chart_object_str( $width, $height, $url, $use_swfobject=true, $base='/kernel/open-flash-chart-2/' )
{
    //
...
function _ofc( $width, $height, $url, $use_swfobject, $base )
{
    ...
    if( !isset( $open_flash_chart_seqno ) )
    {
        ...
        $out[] = '<script type="text/javascript" src="/template/flash/swfobject/swfobject.js"></script>';
    }
    else
    {
    ...
    if( $use_swfobject )
    {
        ...
        $out[] = 'var flashvars = false;';
      	$out[] = 'var params = {};';
        $out[] = 'params.quality = "high";';
        $out[] = 'params.allowFullScreen = "true";';
        $out[] = 'params.FlashVars = "data-file='. $url .'";';
        $out[] = 'swfobject.embedSWF("/template/flash/open-flash-chart.swf", "'.$div_name.'", "'. $width.'", "'.$height.'", "9.0.0", "/template/flash/swfobject/expressInstall.swf", flashvars, params);';
      	$out[] = ' </script>';
		$out[] = '<noscript>';
    }
    ...
    $out[] = '<embed src="/template/flash/open-flash-chart.swf?ofc=' . $url .'" quality="high" bgcolor="#FFFFFF" width="'. $width .'" height="'. $height .'" name="'. $obj_id .'" align="middle" allowScriptAccess="sameDomain" ';
