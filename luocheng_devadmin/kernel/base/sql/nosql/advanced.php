...
class advanced
{
	...
	function addgoods($user_id,$goods_info)
	{
		...
		$tmpbase_goods=dbarr("SELECT a.*,b.goods_name,b.goods_type,b.sub_type FROM `base_goods_ext` a inner join base_goods b on a.goods_id=b.id WHERE a.goods_ext_id=".$goods_info["goods_ext_id"]);
		...
		$mynumeric=array("goods_type"=> 1,"sub_type"=>6,"quality"=>1,"is_sale"=>1,"pack_show"=>1,"wrap_num"=>1,
			"double_click"=>1,"goods_user_id"=>1,"goods_ext_id"=>1,"goods_num"=>1,"user_id"=>1,"binding"=>1,"goods_id"=>1,"goods_state"=>1,"lattice_id"=>1
				,"copper"=>1,"estate"=>1,"time"=>1
		);
		...
		$get_user_goods=$this->mongo->findone("user_goods",array("user_id"=>$user_id,"goods_id"=>$goods_info["goods_id"]),array("goods_user_id"=>true,"goods_num"=>true,"lattice_id"=>true,"goods_name"=>true));
		//判断物品是否可叠加
		...
		{//可叠加 的物品数据一定是相同的，否则不能有叠加属性
			...
			if($run)
			{
				$this->goods_log($user_id,intval($get_user_goods["goods_user_id"]),$get_user_goods["goods_name"],$goods_num);	
			}
			...
			if($run)
			{
				$this->goods_log($user_id,intval($run),$newgoods_info["goods_name"],$goods_num,$newgoods_info["module_id"]);	
			}
		}
	...
	function goods_log($user_id,$goods_user_id,$goods_name,$mmax,$module_id)
	{
		
		$adddata=array(
                  "user_id"=>$user_id,
                  "goods_user_id"=>$goods_user_id,
                  "goods_usre_name"=>$goods_name,
                  "max"=>$mmax,
                  "e"=>"+",
                  "time"=>time(),
            ...
            $this->mongo->insert("goods_act",$adddata);
	}
