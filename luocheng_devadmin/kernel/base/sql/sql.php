...
if(_START_MONGO==1)
{
	...
	{
		$tmp=dbarr("SELECT `server_mongo_ip`,`server_mongo_port`,`mongo_username`,`mongo_password`,`mongo_line` FROM `operators` WHERE `id`=".intval($_SESSION["userid"]),0,"operators");
	...
	$mongo=new ADVANCED;
	$connstr="";
	if(count($config_storage["MONGO"])>=1)
	{
