...
function texts(){  //文本模式
	 ...
	 dates=dates.replace (/<FONT.*?>/g,"");  //去除所有颜色
	 dates=dates.replace (/<font.*?>/g,"");  //去除所有颜色
	 
	 dates=dates.replace (/<B.*?>/g,"");  //去除加粗
	 dates=dates.replace (/<b.*?>/g,"");  //去除加粗
	 
	 dates=dates.replace (/<U.*?>/g,"");  
	 dates=dates.replace (/<u.*?>/g,"");  
	 
	 dates=dates.replace (/<I.*?>/g,"");  
	 dates=dates.replace (/<i.*?>/g,"");  
	 
	 editor.document.body.innerHTML=dates;
...
请输入编码.", "1");
	...
	return;
	var cl=new Array("#FF0000","#FFFFFF","#22ff00","#00ccff","#a525fe","#ff6633","#ffff00","#000000");
	if(url>0 && url<8)
		editor.document.execCommand('forecolor','',cl[url-1]);
...
function SEditor_InsertLink(editor_a){  //超连接
	var url = prompt("请输入超链接的地址.", "event:tavernϢOpenFunction");
	if (url == "event:" || url == null || url == "" || url == "undefined")
	return;
...
function all_delete_InsertLink(editor_a){  //全部删除超连接
    var datess=editor.document.body.innerHTML;
    dates=datess.replace (/<a.*?>/g,"");
	dates=dates.replace (/<A.*?>/g,"");
	editor.document.body.innerHTML=dates;
}
