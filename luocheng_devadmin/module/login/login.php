...
class login extends MODULE implements loginint
{
	...
	public function _sys_login($parameter)
	{
		...
		if($_SESSION["operators"] && $_SESSION["operators"]==$operators)
		{
			$res=1;
		}else{
			$arr=dbarr("SELECT `id` ,`purview`,`default_mysql_db` FROM `operators` WHERE `operators`='".$operators."'",0,"operators");
			$arr_list=dbarr("SELECT `server_id` FROM `server_list` WHERE `operators_id`='".$arr[0]["id"]."' order by server_id DESC",0,"operators");
			if($arr)
			{
				...
				if($this-> pwdcheck($mypurview,$login_user,md5($pwd)))
				{
					...
					$_SESSION["loginuser"]=$loginuser;	//登录人员
					$_SESSION["userid"]=$arr[0]["id"];
					$_SESSION["server_id"]=$arr_list[0]["server_id"];
					$_SESSION["purview"]=$this->mypurview($mypurview,$login_user);
