...
class itask extends module
{
	...
	function experience()
	{
		$base1_hero_lv=dbarr("SELECT * FROM `base1_hero_lv` WHERE 1");
		$lv=array();
		foreach($base1_hero_lv as $value)
		{
			$lv[$value["id"]]=$value["experience_top"];
		}
		...
		{
			$map[$value["id"]]=$value["experience"];
		}
		
		$base2_monster_list=dbarr("SELECT * FROM `base2_monster_list` WHERE 1");
		$monster_list=array();
		foreach($base2_monster_list as $value)
		{
			$monster_list[$value["id"]]=$value;
		}
		
		$base3_task=dbarr("SELECT a.id,a.`name`,a.`experience_award`,b.condition_type,b.condition FROM `base3_task` a inner join base3_task_condition b on a.id=b.task_id  WHERE a.type=1");
		return array($map,$lv,$monster_list,$base3_task);
		
	}
	
	function upexp($id,$field,$value)
	{
		return sql("UPDATE `base3_task` SET  `experience_award` =  '$value' WHERE `id` =".$id);
	}
