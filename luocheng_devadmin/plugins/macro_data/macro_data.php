...
//		require_once parse_tmp("/login/login.html");//引入模版
	...
	{
		$obj->_bug=0;
		$post=isset($obj->post["nickname"])?1:0;
		$post=($post==0)?(isset($obj->get["query"])?1:0):$post;
		if($post)
		{
			$cont=new CONT("macro_data");
			$obj->post["table"]=isset($obj->post["table"])?$obj->post["table"]:"user_info";
			$nickname=isset($obj->post["nickname"])?$obj->post["nickname"]:$obj->get["nickname"];
			$user_name=isset($obj->post["user_name"])?$obj->post["user_name"]:$obj->get["user_name"];
	...
	function update_user_info($obj)
	{
		...
		if($obj->post)
		{
			...
			$user_info=$cont->obj->update_user_info($table,intval($obj->post["user_id"]),$obj->post);
		}
		...
		if(isset($obj->post["actquery"]) and $obj->post["actquery"])
		{
			$obj->post["nickname"]=$obj->post["actquery"];
		}
...
//封用户ID列表
	...
	function huodong($obj)
	{
		...
		$huodong_list=array();
		foreach($_huodong[0] as $key=>$value)
		{
			$huodong_list[$value["id"]]=$value;
		}
		
		foreach($_huodong[1] as $key=>$value)
		{
