...
class imap extends module
{
	...
	function edit_get_map_1($mapid)
	{
		$base_map_word=dbarr("SELECT id,name,icon,icon_xy FROM `base2_map1` WHERE `status`=1 and type<=2 and country=$mapid order by id desc");
		echo json_encode($base_map_word);
		
	}
	
	function edit_upxy($mapid,$xy)
	{
		sql("UPDATE  `base2_map1` SET  `icon_xy` = '".str_replace("px","",$xy)."' WHERE `id` =".$mapid);
	}
}
?>